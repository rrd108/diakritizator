...
for($i = 0; $i < 50; $i++){
	...
	try{
		...
		foreach($szavak as $szo){
			if(mb_strlen($szo)){
				...
				foreach($dbh->query('SELECT * FROM szoparok2 WHERE sima = "' . $szo . '"') as $row){
	...
	$test[] = (microtime(true)-$startTime);
	//print '<p contenteditable>' . nl2br($szoveg) . '</p>';
	/*$fp = fopen('/home/rrd/public_html/diakritizator/time.csv', 'a');
	fwrite($fp, (microtime(true)-$startTime). "\n");
	fclose($fp);*/
}
var_dump($test);
print '<p>Átlagos idő: '.array_sum($test)/count($test).'</p>';
?>
</section>
</body>
</html>
